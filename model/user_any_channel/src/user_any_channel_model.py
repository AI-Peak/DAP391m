...
def main():
    ...
    print("Preparing user-level features...")
    df_user = (
        df_enc.groupby("User ID")[channel_cols]
        .max()
        .reset_index()
        .merge(df_jr[["User ID", "Converted"]], on="User ID", how="inner")
    )
    
    print("Fitting User-Any-Channel Logistic Regression...")
    fitted_user, metrics_user = fit_logit("user_any_channel", df_user[channel_cols], df_user["Converted"])
    
    # Save metrics
    metrics_df = pd.DataFrame([metrics_user]).round(4)
    metrics_df.to_csv(OUTPUT_DIR / "user_any_channel_metrics.csv", index=False)
    
    # Save coefficients
    conf = fitted_user.conf_int()
    conf.columns = ["ci_low", "ci_high"]
    coefficients = pd.DataFrame({
        "coef": fitted_user.params,
        "odds_ratio": np.exp(fitted_user.params),
        "p_value": fitted_user.pvalues,
    }).round(4)
    coefficients.to_csv(OUTPUT_DIR / "user_any_channel_coefficients.csv")
