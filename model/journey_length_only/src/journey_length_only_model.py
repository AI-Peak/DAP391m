...
def main():
    ...
    print("Fitting Journey-Length-Only Logistic Regression...")
    # Using only N_Touchpoints as predictor
    fitted_len, metrics_len = fit_logit("journey_length_only", df_jr[["N_Touchpoints"]], df_jr["Converted"])
    
    # Save metrics
    metrics_df = pd.DataFrame([metrics_len]).round(4)
    metrics_df.to_csv(OUTPUT_DIR / "journey_length_metrics.csv", index=False)
    
    # Save coefficients
    conf = fitted_len.conf_int()
    conf.columns = ["ci_low", "ci_high"]
    coefficients = pd.DataFrame({
        "coef": fitted_len.params,
        "odds_ratio": np.exp(fitted_len.params),
        "p_value": fitted_len.pvalues,
    }).round(4)
    coefficients.to_csv(OUTPUT_DIR / "journey_length_coefficients.csv")
